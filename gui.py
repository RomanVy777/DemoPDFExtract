...
def construir_texto_relevante(texto):
    """
    Construye el texto a enviar a Claude priorizando las secciones relevantes.

    CAMBIOS respecto a la versión anterior:
    - Se amplía el contexto alrededor de cada línea importante (±5 líneas en vez de ±3/+5).
    - Se eliminan duplicados exactos pero se preservan líneas similares con distinto valor
      (por ejemplo dos coberturas con el mismo nombre de campo pero distinto importe).
    - Si el texto completo cabe en MAX_CARACTERES_TEXTO_RELEVANTE, se devuelve completo
      sin filtrar, evitando descartar datos útiles innecesariamente.
    - Se quita la anotación "[línea N]" porque puede confundir al modelo.
    """
    lineas_originales = texto.splitlines()
    lineas = [l.strip() for l in lineas_originales if l.strip()]

    # Si el texto completo cabe, lo devolvemos tal cual (sin filtrar)
    texto_completo = "\n".join(lineas)
    ...
    # Siempre incluir las primeras N líneas (cabecera del documento)
    for i in range(min(MAX_LINEAS_INICIALES, len(lineas))):
        posiciones.add(i)

    # Incluir líneas importantes con contexto ampliado (±5 líneas)
    ...
    for pos in posiciones_ordenadas:
        linea = lineas[pos]
        # Deduplicar solo líneas EXACTAMENTE iguales (no por similitud)
        if linea in vistos:
            continue
        vistos.add(linea)
