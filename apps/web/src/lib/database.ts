...
//   TypeScript replica EXACTAMENTE esta protección con escaparIdentificador()
...
/**
 * Envuelve un identificador SQL Server en corchetes y escapa corchetes internos.
 * Equivale a proteger_nombre_mysql() de Python.
 */
function escaparIdentificador(nombre: string): string {
...
//     escaparIdentificador(). Los valores van como parámetros (?).
//
...
async function obtenerColumnasExistentes(nombreTabla: string): Promise<Set<string>> {
  ...
  `;
  return new Set(columnas.map((col) => col.COLUMN_NAME));
}
...
async function crearTablaDinamicaSiNoExiste(nombreTabla: string): Promise<void> {
  const nombreEscapado = escaparIdentificador(nombreTabla);

  await prisma.$executeRawUnsafe(`
    IF NOT EXISTS (
      SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '${nombreTabla}'
    )
    BEGIN
      CREATE TABLE ${nombreEscapado} (
        id INT IDENTITY(1,1) PRIMARY KEY,
        id_documento INT,
        fecha_importacion DATETIME2,
...
async function crearColumnasSiNoExisten(
  ...
  const nombreTbEscapado = escaparIdentificador(nombreTabla);

  for (const columna of columnas) {
    if (!columnasExistentes.has(columna)) {
      const columnaEscapada = escaparIdentificador(columna);
      await prisma.$executeRawUnsafe(`
        ALTER TABLE ${nombreTbEscapado}
        ADD ${columnaEscapada} NVARCHAR(MAX)
      `);
    }
  }
}
...
async function insertarFilaDinamica(
  ...
  const columnasSql = columnas
    .map((c) => escaparIdentificador(c))
    .join(", ");

  // SQL Server usa @p1, @p2... como placeholders (no ? como MySQL)
  const placeholders = columnas.map((_, i) => `@p${i + 1}`).join(", ");
